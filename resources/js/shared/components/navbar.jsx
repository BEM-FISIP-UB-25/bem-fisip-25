...
const Navbar = () => {
    ...
    return (
        ...
        <nav className=" py-[2vw] lg:h-[7vw] w-full  bg-gradient-to-r from-[#FFEFE2] from-0% via-[#FFE2C5] via-7% to-[#FF9A27] to-59% flex items-center justify-between  sm:rounded-b-3xl relative shadow-lg px-[2vw] overflow-hidden z-30 absolute">
            <img src={shapes} alt="Shapes" className="absolute top-0 left-0 w-full h-full z-0" />
            
            {/* Logo dan Judul (Link ke Beranda) */}
            <Link href="/" className="flex flex-row items-center gap-x-[1vw] z-20">
                <img src={eskalasi} alt="Eskalasi" className="w-[8vw] md:w-14" />
                <div className="flex flex-col">
                    <h1 className="monumentExtendedUltrabold text-[#23235F] text-[5vw] sm:text-3xl lg:text-[2.5vw] ">BEM FISIP UB 2025</h1>
                    <h3 className="monumentExtendedRegular text-white text-[3vw] sm:text-lg lg:text-[1.6vw] ">KABINET ESKALASI MIMPI</h3>
                </div>
            </Link>
                    ...
                    >
                        ...
                        >
                            <div className="flex justify-between items-center mb-12">
                                <h2 className="text-white text-2xl monumentExtendedUltrabold">MENU</h2>
                            ...
                            <ul className="flex flex-col gap-y-6 arialBlack text-lg">
                                ...
                                <NavLink href="/" onClick={() => setIsOpen(false)}>BERANDA</NavLink>
                                <NavLink href="/profil" onClick={() => setIsOpen(false)}>PROFIL</NavLink>
                                <NavLink href="/struktur" onClick={() => setIsOpen(false)}>STRUKTUR</NavLink>
                                <NavLink href="/lembaga" onClick={() => setIsOpen(false)}>LEMBAGA</NavLink>
                                <NavLink href="/artikel" onClick={() => setIsOpen(false)}>ARTIKEL</NavLink>
                                <NavLink href="/informasi" onClick={() => setIsOpen(false)}>INFORMASI</NavLink>
                                <NavLink href="/layanan" onClick={() => setIsOpen(false)}>LAYANAN</NavLink>
                            </ul>
                        </motion.div>
                    </motion.div>
                )}
